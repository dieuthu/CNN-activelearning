...
class ReadlevelClassifier():
    ...
    def __add_sentences(self, docs, max_sent):
        out = []
        for doc in docs:
            l = self.__to_word_idx(doc) #add sentence indices to this doc
            if len(doc) < max_sent:
                for i in range(len(doc), max_sent):
                    tmp = []
                    for j in range(0, self.params["MAX_SENT_LEN"]):
                        tmp.append(self.params["VOCAB_SIZE"]+1)
                    l.append(tmp)
            out.append(l)
        return out
    ...
    def fit(self, X, y, dev_X=[], dev_y=[], test_X=[], test_y=[]):
        """
        X: a list of documents, each is a list of words
        y: a list of corresponding classes
        ...
        if (self.SENT_AVG):
            ...
            for doc in inp: #for each doc
                ...
                for s in x_doc:
                    x_av+=s
                x_av=x_av/len(x_doc)
                #print("Org x_av size ", x_av.size())
                #x_av = x_av.view(-1,1,self.FRE_DIM * self.MAX_SENT_LEN)
                #print('x_av')
                #print(x_av.size())
                #print('x_docs')
                #print(x_docs.size())
                if (x_docs is None):
                    ...
                    #print(x_docs.size())
                    x_docs = torch.cat((x_docs,x_av),0)
            #print('final x-docs size ',x_docs.size())
            print(x_docs)
